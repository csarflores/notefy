...
interface ProjectWithFiltersProps {
  tasks: ITask[];
  projectId: string;
  projectUsers: IUser[];
  projectTags: ITag[];
...
export default function ProjectWithFilters({ 
  ...
  projectUsers,
  projectTags
}: ProjectWithFiltersProps) {
  const { data: session } = useSession();
  const [currentFilter, setCurrentFilter] = useState<FilterType>('all');

  // Si no hay tags en el proyecto, extraer todas las tags únicas de las tareas
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-4 sm:py-5">
      {tasks.length === 0 ? (
        // Estado vacío - sin tareas en el proyecto
